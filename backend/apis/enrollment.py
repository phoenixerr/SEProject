...
api = Namespace('Enrollments', description='Collection of enrollment endpoints')
...
class StudentEnrollmentAPI(Resource):
    @jwt_required()
    @api.doc(description = "Return if a student of specified user ID is enrolled to course of specified course ID.")
    ...
    @api.doc(description = "Add a student of specified user ID to course of specified course ID.")
    ...
    @jwt_required()
    @api.doc(description = "Delete a student of specified user ID from course of specified user ID.")
...
class InstructorTeachAPI(Resource):
    @jwt_required()
    @api.doc(description = "Allows to check if the instructor with specified user ID is teaching course with specified ID.")
    def get(self, user_id, course_id):
        self_id = get_jwt_identity()
        user = User.query.get(self_id)
        if not user:
    ...
    @jwt_required()
    @api.doc(description = "Add instructor with specified user ID to teach course of specified course ID.")
    @api.expect(enrollment_parser)
    def post(self, user_id, course_id):
        self_id = get_jwt_identity()
        user = User.query.get(self_id)
    ...
    @jwt_required()
    @api.doc(description = "Delete the instructor with specified user ID from course with specified ID.")
    def delete(self, user_id, course_id):
        self_id = get_jwt_identity()
        user = User.query.get(self_id)
        if not user:
